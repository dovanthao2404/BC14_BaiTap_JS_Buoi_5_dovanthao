...
function totalLevel3(kw) {
  var totalVevel2 = totalLevel2(100);
...
function totalLevel5(kw) {
  var totalLevel4 = totalLevel4(350);
...
function getTotalMoney(kw) {
  if (kw <= 50) {
    return totalLevel1(kw);
  }
  if (kw <= 100) {
    return totalLevel2(kw);
  }
  if (kw <= 200) {
    return totalLevel3(kw);
  }
  if (kw <= 350) {
    return totalLevel4(kw);
  }
  return totalLevel5(kw);
}
...
getEle("btnTotalAmount").addEventListener("click", function () {
  ...
  if (result) {
    ...
    getEle('resultAmount').innerHTML = "Tổng tiền là: " + totalMoney + " VND";
  }
})